...
"""
#ニューラルネットと比較
#パイプラインが使えないので、スケーリングと学習、testデータで判定、scoreの算出をしている

nn = NeuralNetMLP(n_output=7,
                  n_features=features,#X_train_std.shape[1],
                  n_hidden=10,
                  l2=0,
                  l1=0.5,
                  epochs=1000,
...
nn.fit(X_train_std, y_train, print_progress=True)
#elapsed_time = time.time() - start

y_pred = nn.predict(X_test_std)
print('1回のAccuracyNN: %.2f' % accuracy_score(y_test, y_pred))
...
#############################################################################
"""
print('Using cross_val_score↑の簡略化コード')
...
"""
sci_kit learn 20.0以降はcrossvalidation廃止のため以下になる(途中保存)
